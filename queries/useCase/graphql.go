...
func (uC *UseCase)GetAllUsersByOrganisation(payloadToken string)(*[]response.User,string,error){
	tkn,err := token.CheckToken(payloadToken)
	if err!=nil {
		return nil,"",err
	}
	...
	users,err := uC.queriesRepository.GetAllUsersByOrganisationId(tkn.OrganisationId)
	if err != nil{
		return nil,newToken,err
	}
